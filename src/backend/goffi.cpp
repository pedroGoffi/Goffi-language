...
namespace Goffi{
  void compile_program(std::vector<std::pair<VR, Token>>program, std::string outputFilePath){
      assert(NUM_OF_OPERANDS == 35 && "Exhaustive time handling operand, please update the compile_program in ");
      ...
      while( ip != program.end()){
  		...
  		break;
              ...
              case OP_DO: 
                  ...
                  out <<  "   ;; ---- do\n"
                      <<  "   pop rax\n"
                      <<  "   test rax, rax\n"
                      <<  "   jz __label_num__" << ip->first.operand  << "\n"                
                      ;
                  ++ip;
                  break;
              case OP_END:
  		...
  		out <<	"   ;; ---- push str\n"
  		    ...
  		    <<	"   push rax\n"
  		    <<	"   push word_string__" << ip->first.operand << "\n"
  		    ;
  		++ip;
  		break;
  	    case OP_DIVMOD: 
  		...
  		makeLabel;
  		out <<	"   ;; ----- else\n"
  		    <<	"   jmp __label_num__" << ip->first.operand  << "\n"                
  		    ;
  		++ip;
  		break;
  	    case OP_WHILE:
